...
export default function JobResults({ jobId, currentTitle, onMetaLoaded }: JobResultsProps) {
  ...
  const fetchData = useCallback(async () => {
    const [{ data: outputsData }, { data: jobData }] = await Promise.all([
      ...
      supabase.from("jobs").select("language_detected, summary_language, duration_seconds, file_name, created_at, recorded_at, recorded_at_source, speech_model, speaker_names, title").eq("id", jobId).maybeSingle(),
    ]);
    ...
    <div className="space-y-4 animate-page-enter">
      ...
      <Tabs defaultValue="transcript" className="w-full">
        <TabsList className="w-full justify-start rounded-xl bg-muted/50 p-1 h-auto">
          ...
          <TabsTrigger value="summary" className="rounded-lg gap-1.5 text-sm data-[state=active]:bg-background data-[state=active]:shadow-sm"><Sparkles className="w-3.5 h-3.5" />{t("jobResults.summary")}</TabsTrigger>
          <TabsTrigger value="questions" className="rounded-lg gap-1.5 text-sm data-[state=active]:bg-background data-[state=active]:shadow-sm"><HelpCircle className="w-3.5 h-3.5" />{t("jobResults.questions")}</TabsTrigger>
        </TabsList>
