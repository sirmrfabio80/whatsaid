...
export default function ExportButton({ data, disabled }: ExportButtonProps) {
  ...
  const handleExport = async (format: ExportFormat) => {
    ...
    try {
      switch (format) {
        case "txt":
          downloadString(buildTxt(data), `${data.title}.txt`, "text/plain;charset=utf-8");
          break;
        case "json":
          downloadString(buildJson(data), `${data.title}.json`, "application/json");
          break;
        case "doc":
