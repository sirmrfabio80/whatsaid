...
export async function generatePdfBlob(data: CanonicalExportData): Promise<Blob> {
  ...
  try {
    ...
    for (let p = 1; p <= pageCount; p++) {
      ...
      pdf.text(footerText, textStartX, footerY + 1);

      // Clickable link over the footer text area
      const textWidth = pdf.getTextWidth(footerText);
      pdf.link(textStartX, footerY - 2, textWidth, 5, { url: WHATSAID_URL });

      // Page number
      pdf.text(`${p} / ${pageCount}`, PAGE_WIDTH_MM - MARGIN_MM, footerY + 1, { align: "right" });
    }
